...
#results { display:grid; grid-template-columns:repeat(auto-fill,minmax(180px,1fr)); gap:15px; padding:20px; }
...
function renderResults(movies) {
  ...
  movies.forEach(m => {
    const files = m.files || [];
    const mp4 = files.find(f => f.name && f.name.endsWith('.mp4'));
    if(!mp4) return;
    const videoUrl = \`https://archive.org/download/\${m.identifier}/\${mp4.name}\`;
